...
export const PagesEditor = forwardRef<PagesEditorRef, PagesEditorProps>(({
  ...
  useEffect(() => {
    ...
    const onTouchStart = (e: TouchEvent) => {
      ...
      // 1. WHILE APPLE PENCIL IS ON GLASS: All finger touches are resting palm -> 100% REJECT
      if (hasStylus || isPenActive.current) {
        e.preventDefault();
        return;
      }

      // 2. TWO-FINGER PINCH-TO-ZOOM & PAN:
      if (e.touches.length === 2) {
        const t1 = e.touches[0];
        const t2 = e.touches[1];
        const dist = Math.hypot(t2.clientX - t1.clientX, t2.clientY - t1.clientY);

        // Discard accidental double-touches from a single finger crease (< 25px)
        if (dist >= 25) {
          isPinching = true;
          pinchStartDist = dist;
      ...
      // 3. FLAT PALM SLAP OR 3+ TOUCH REST (Hand resting without stylus):
      if (e.touches.length >= 3 || Array.from(e.touches).some(isFlatPalmTouch)) {
        e.preventDefault();
        return;
      }

      // 4. Inter-stroke handwriting pause check: if stylus was lifted < 250ms ago and contact is wide
      if (lastPenTime.current > 0 && Date.now() - lastPenTime.current < 250) {
        const t = e.touches[0];
        const rx = t.radiusX || 0;
        const ry = t.radiusY || 0;
        const w = (t as any).width || 0;
        const h = (t as any).height || 0;
        if (rx > 36 || ry > 36 || w > 72 || h > 72) {
          e.preventDefault();
          return;
        }
      }

      // 5. Normal single-finger touch: Allow native 120Hz momentum scrolling!
    };
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      if (e.touches.length === 2) {
        ...
        if (!isPinching || pinchStartDist < 25) {
          isPinching = true;
          pinchStartDist = Math.max(currentDist, 25);
          pinchStartZoom = zoomRef.current;
          pinchStartMid = {
            x: (t1.clientX + t2.clientX) / 2,
            y: (t1.clientY + t2.clientY) / 2,
        ...
        // Focal zoom calculation keeps the content point under the fingers stationary
        const containerRect = container.getBoundingClientRect();
        const currentMid = {
          x: (t1.clientX + t2.clientX) / 2,
          y: (t1.clientY + t2.clientY) / 2,
        };
        const focalX = pinchStartMid.x - containerRect.left;
        const focalY = pinchStartMid.y - containerRect.top;
        const contentX = (pinchStartScroll.left + focalX) / pinchStartZoom;
        const contentY = (pinchStartScroll.top + focalY) / pinchStartZoom;
        const panShiftX = currentMid.x - pinchStartMid.x;
        const panShiftY = currentMid.y - pinchStartMid.y;

        container.scrollLeft = contentX * newZoom - focalX - panShiftX;
        container.scrollTop = contentY * newZoom - focalY - panShiftY;
        return;
      }

      // 2. Reject flat palm slap or 3+ touches
      if (e.touches.length >= 3 || Array.from(e.touches).some(isFlatPalmTouch)) {
        e.preventDefault();
        return;
      }

      // 3. Prevent scroll jump if 1 finger remains right after pinch end
      if (Date.now() - lastPinchEndTime < 100) {
        e.preventDefault();
        return;
      }

      // 4. Normal single-finger touch: scrolls natively with 120Hz momentum!
    };
    ...
    const handleGlobalPenUp = (e: PointerEvent) => {
    ...
    window.addEventListener('pointerup', handleGlobalPenUp);
    window.addEventListener('pointercancel', handleGlobalPenUp);
    ...
    return () => {
      ...
      window.removeEventListener('pointerup', handleGlobalPenUp);
      window.removeEventListener('pointercancel', handleGlobalPenUp);
    };
  }, []);
  ...
  function handlePointerDown(e: React.PointerEvent, pageId: string) {
    ...
    if (e.pointerType === 'pen') {
      ...
      if (isPenActive.current || isPalm) {
        // Resting palm or touch while pencil is actively writing: REJECT COMPLETELY!
        e.preventDefault();
        e.stopPropagation();
        return;
      }
  ...
  function handlePointerMove(e: React.PointerEvent, pageId: string) {
    ...
    if (tool === 'shapes' && shapeStartRef.current) {
      ...
      if (type === 'rect') ctx.strokeRect(start.x, start.y, w, h);
        ...
        const arrowAngle = Math.atan2(endY - start.y, endX - start.x);
        const headlen = 15;
        ctx.moveTo(start.x, start.y);
    ...
    if (tool === 'ruler' && holdStartPos.current) {
      ...
      if (snapped.isSnapped) {
        ...
        const midY = (holdStartPos.current.y + snapped.y) / 2 - 14;

        ctx.fillStyle = 'rgba(0, 122, 255, 0.88)';
        ctx.beginPath();
        ctx.roundRect(midX - textW / 2 - 6, midY - 9, textW + 12, 18, 9);
        ctx.fill();
    ...
    if (tool === 'shapes' && shapeStartRef.current) {
      ...
      if (Math.abs(w) > 5 || Math.abs(h) > 5) {
        ...
        if (type === 'rect') {
          ...
          const arrowAngle = Math.atan2(endY - start.y, endX - start.x);
          const headlen = 15;
          const x1 = endX - headlen * Math.cos(arrowAngle - Math.PI / 6);
          const y1 = endY - headlen * Math.sin(arrowAngle - Math.PI / 6);
          const x2 = endX - headlen * Math.cos(arrowAngle + Math.PI / 6);
          const y2 = endY - headlen * Math.sin(arrowAngle + Math.PI / 6);
  ...
  return (
    <div className="relative w-full h-full overflow-hidden select-none">
      ...
      >
        <div
          className={`flex flex-col items-center gap-8 md:gap-10 w-fit min-w-full transition-[padding] duration-300 ease-out ${
            toolbarPosition === 'top'
              ? focusMode
      ...
      {/* Top Blur Blend (Silky Apple-grade progressive fade) */}
      <div
        className={`pointer-events-none absolute top-0 left-0 right-0 h-24 md:h-32 z-20 transition-opacity duration-300 ease-out ${
          canScrollUp ? 'opacity-100' : 'opacity-0'
        }`}
        style={{
          background: `linear-gradient(to bottom, 
            var(--canvas-bg) 0%, 
            color-mix(in srgb, var(--canvas-bg) 96%, transparent) 12%,
            color-mix(in srgb, var(--canvas-bg) 88%, transparent) 24%,
            color-mix(in srgb, var(--canvas-bg) 75%, transparent) 38%,
            color-mix(in srgb, var(--canvas-bg) 56%, transparent) 54%,
            color-mix(in srgb, var(--canvas-bg) 35%, transparent) 70%,
            color-mix(in srgb, var(--canvas-bg) 15%, transparent) 85%,
            transparent 100%)`,
      ...
      {/* Bottom Blur Blend (Silky Apple-grade progressive fade) */}
      <div
        className={`pointer-events-none absolute bottom-0 left-0 right-0 h-24 md:h-32 z-20 transition-opacity duration-300 ease-out ${
